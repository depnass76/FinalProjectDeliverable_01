...
public class SwingAnimatorBuilder implements AnimatorBuilder {
	...
	public Animator getAnimator() {
		...
		Animator returnValue = new SwingAnimator(_painter, "Traffic Simulator", VP.displayWidth, VP.displayHeight,
				VP.displayDelay);
		_painter = null;
		return returnValue;
	}
	...
	public static class MyPainter implements SwingAnimatorPainter {
		...
		public void paint(Graphics g) {
			...
			for (Element<Light> e : _lightElements) {
				if (e.x.getState() == state.GREEN) {
					g.setColor(Color.GREEN);
				} else if (e.x.getState() == state.YELLOW) {
			...
			for (Element<Road> e : _roadElements) {
				...
				for (Car d : e.x.getCars().toArray(new Car[0])) {
					if (d != null) {
						g.setColor(d.getCarColor());
						XGraphics.fillOval(g, e.t, d.getBackPos() * (MP.roadLength / e.x.getLength()), 0,
								d.getLength() * (MP.roadLength / e.x.getLength()), VP.elementWidth);
					}
				}
			}
		}
